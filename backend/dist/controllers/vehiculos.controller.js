...
export const assignDriverToVehicle = async (req, res) => {
  ...
  try {
    const verifyDriver = await Persona.findOne({
      cedula
    });
    const verifyVehicle = await Vehiculo.findOne({
      placa
    });
    if (!verifyDriver || !verifyVehicle) {
      return res.status(404).json({
        message: 'Este conductor (o vehículo) no se encuentra(n) registrado(s)...!'
      });
    }
    const updateData = {
    ...
    const assignDriver = await Vehiculo.findOneAndUpdate(verifyVehicle._id, {
      $set: updateData
    }, {
      new: true
    });
    ...
    await Persona.findOneAndUpdate(verifyDriver._id, {

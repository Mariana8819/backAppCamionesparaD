...
const materialesPermitidos = ['Arena', 'Grava', 'Tierra', 'Piedra', 'Escombros', 'Concreto', 'Asfalto', 'Roca triturada', 'Ripio', 'Minerales (por ejemplo, carbón o cobre)', 'Basura (en proyectos de demolición o limpieza)', 'Material orgánico (en proyectos de paisajismo, como tierra vegetal)', 'Otro'];
export const createVolqueta = async (req, res) => {
  try {
    ...
    if (!materialesPermitidos.includes(material)) {
      return res.status(400).json({
        message: `Material inválido. Debe ser uno de los siguientes: ${materialesPermitidos.join(', ')}.`
      });
    }
    ...
    await Persona.findOneAndUpdate(driver._id, {
      $push: {
    ...
    //
    await Vehiculo.findOneAndUpdate(vehicle._id, {
      $push: {
        volquetas: newVolqueta._id
      }
    }, {
...
export const getVolqueta = async (req, res) => {
  try {
    ...
    const planilla = await Volqueta.find(query).populate('conductor').populate('placas');
...
export const putVolqueta = async (req, res) => {
  try {
    ...
    if (!findPlanilla) return res.status(404).json({
    ...
    const updatedVolqueta = await Volqueta.findOneAndUpdate(query, updateData, {
      new: true,

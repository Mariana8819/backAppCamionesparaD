...
export async function plantillaTanqueo(data) {
  try {
    ...
    const FormattedDate = new Date(data.fecha_tanqueo).toLocaleDateString('es-VE', {
      day: '2-digit',
      month: '2-digit',
      year: 'numeric'
    });
    ...
    const pdfPath = path.join(pdfDir, `recibo_tanqueo_${data.n_recibo}.pdf`);
    ...
    const options = {
      ...
      out_dir: pdfDir,
      // Carpeta de salida...
      out_prefix: `recibo_tanqueo_${data.n_recibo}`,
    ...
    const convertedFiles = fs.readdirSync(pdfDir).filter(file => file.startsWith(options.out_prefix) && file.endsWith('.png'));

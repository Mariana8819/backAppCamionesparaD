...
export function plantillaVolquetas(volquetas, res) {
  ...
  try {
    ...
    volquetas.forEach(volqueta => {
      ...
      const detallesGenerales = [[`Fecha: ${volqueta.fecha.toLocaleDateString()}`, `Placa: ${volqueta.placa.placa}`, `VOL.MTS3: ${volqueta.volmts3}`], [`Cliente: ${volqueta.cliente}`, `Conductor: ${volqueta.conductor.nombres}${volqueta.conductor.apellidos}`, `N_viajes: ${volqueta.n_viajes}`], [`Hora Inicio: ${volqueta.hora_inicio.toLocaleTimeString()}`, `Hora Final: ${volqueta.hora_final.toLocaleTimeString()}`, `Total horas: ${volqueta.total_horas}`], [`KM Inicial: ${volqueta.km_inicial}`, `KM Final: ${volqueta.km_final}`, `KM recorridos: ${volqueta.total_km_dia}`]];
      ...
      const datosAdicionales = [["Material", volqueta.material], ["Lugar De Cargue", volqueta.lugar_de_cargue], ["Lugar De Descargue", volqueta.lugar_de_descargue], ["Honorarios", volqueta.honorarios], ["Observacion", volqueta.observacion]];
